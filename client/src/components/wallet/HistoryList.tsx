      ...
      <div className="flex flex-col my-4">
        {
          data.map((item: TransactionHistoryType) => (
            <div key={item.id} className="flex justify-between items-center py-2">
              ...
              <p className="text-xs md:text-xl w-4/12">{item.date.toDateString()}</p>
              
              <div className="flex flex-col w-3/12">
                <h1 className="text-sm md:text-2xl">{item.currency} {item.coin.toUpperCase()}</h1>
                <p className="text-xs md:text-xl text-[#B9B9B9]">${(item.currency * cryptoCurrency[item.coin.toUpperCase()]).toLocaleString()}</p>
              </div>

              <div className="flex items-center w-3/12">
                <span className={`w-2 h-2 md:w-3 md:h-3 mx-2 rounded-full bg-${status[item.status]}-400`} />
                <h1 className="text-xs md:text-xl">{item.status}</h1>
              </div>
            </div>
          ))
        }
